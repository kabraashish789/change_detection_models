...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets, smooth=7.48684e-05):
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.735583, gamma=1):
...
def main():
    ...
    model = RAUNet(in_channels=6, out_channels=1).to(DEVICE)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_dice, avg_iou = check_accuracy(
            test_loader,
            model,
            device=DEVICE,
            save_folder="debug_plots/"
        )
