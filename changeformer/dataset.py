...
class CDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Get current image path (t2: the "after" image)
        img_path_t2 = self.image_paths[idx]
        
        # Construct the previous image path (t1: the "before" image)
        ...
        # Change: use key "image" for the primary (after) image, and use "image_t1" for the before image.
        if self.transform is not None:
            augmentations = self.transform(image=image_t2, image_t1=image_t1, mask=mask)
            image_t2 = augmentations["image"]
            image_t1 = augmentations["image_t1"]
            mask = augmentations["mask"]

        # IMPORTANT: Return the images in the order (before, after, mask)
        # where 'before' is image_t1 and 'after' is image_t2.
