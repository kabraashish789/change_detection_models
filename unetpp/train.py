...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets, smooth=3.83816e-06):
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=0.67529, gamma=3):
...
def get_loaders(train_txt_file, test_txt_file, batch_size, train_transform, test_transform, num_workers, pin_memory):
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=pin_memory, shuffle=False
    )
...
def main():
    ...
    model = UNetPlusPlus(in_channels=6, num_classes=1, deep_supervision=True).to(DEVICE)
    ...
    train_loader, test_loader = get_loaders(
        TRAIN_TXT_FILE, TEST_TXT_FILE, BATCH_SIZE, train_transform, test_transforms, NUM_WORKERS, PIN_MEMORY
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        avg_dice, avg_iou = check_accuracy(test_loader, model, device=DEVICE, save_folder="debug_plots/")
